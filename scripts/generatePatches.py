...
def extract_patches(image, outname,patchshape, overlap_allowed=0.5, cropvalue=None,
    ...
    for i,pat in enumerate(patches):
        cv2.imwrite('output/'+outname+'_'+str(i)+'.bmp',pat)
...
def extract_patches_from_dir(directory, patchsize, overlap_allowed=0.5,
    ...
    for fname in os.listdir(directory):
        if fname[-4:] == '.bmp':
            ...
            image = cv2.imread(os.path.join(directory, fname),cv2.IMREAD_GRAYSCALE)
            # Extract patches from the image.
            output[outname] = extract_patches(image, outname,patchsize,
                                              overlap_allowed,
...
op = extract_patches_from_dir('../dataset/rgb91/',(36,36),overlap_allowed=0.166)